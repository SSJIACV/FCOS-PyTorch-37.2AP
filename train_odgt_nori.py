...
import torch
from dataset.odgt_nori_dataset import Odgt_Nori_Dataset
import math, time
...
# rlaunch --cpu=12 --gpu=8 --memory=$((120*1024)) --max-wait-time 10h --preemptible=no --charged-group v_tracking -- python3 train_odgt_nori.py
...
save_path = Config_nori_myself.save_path
os.makedirs(save_path, exist_ok=True)
train_odgt = Config_nori_myself.train_odgt
...
train_dataset = Odgt_Nori_Dataset(train_odgt=train_odgt,resize_size=resize_size,CLASSES_NAME = CLASSES_NAME,is_train = True, augment = transform)
model = FCOSDetector(mode="training",config=Config_nori_myself).cuda()
model = torch.nn.DataParallel(model)

if Config_nori_myself.pretrained_coco:
    model.load_state_dict(torch.load(Config_nori_myself.coco_weight_path))
...
WARMPUP_STEPS = Config_nori_myself.WARMPUP_STEPS

GLOBAL_STEPS = 1
LR_INIT = Config_nori_myself.LR_INIT
LR_END = 2e-5
